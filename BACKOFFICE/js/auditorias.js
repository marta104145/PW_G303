...
function carregarOcorrenciasAceites() {
  ...
  aceites.forEach((ocorrencia, index) => {
    ...
    if (ocorrencia.prioridade) {
      ...
      const prioridadeMatch = ocorrencia.prioridade.toString().match(/^(\d+)/)
      const nivel = prioridadeMatch
        ? Number.parseInt(prioridadeMatch[1])
        : ["Alta", "Média", "Baixa"].includes(ocorrencia.prioridade)
          ? { Alta: 4, Média: 3, Baixa: 2 }[ocorrencia.prioridade]
          : 3
      ...
      if (!isNaN(nivel) && nivel >= 1 && nivel <= 5) {
        ...
        prioridadeDisplay = ocorrencia.prioridade
      }
    }
    ...
    if (dados.prioridade.includes("prioridade-badge")) {
      ...
      const nivel = prioridadeMatch
        ...
        : ["Alta", "Média", "Baixa"].includes(dados.prioridade)
          ? { Alta: 4, Média: 3, Baixa: 2 }[dados.prioridade]
          : 3
